const
	...
	trackTitle = document.querySelector( 'h2' ),
	trackArtist = document.querySelector( 'p' ),
	API = {
...
if ( !trackArtist || !trackTitle ) throw new Error( 'Missing elements' );

const fetchResponse: FetchUserFunction = async (userId) => {
	try {
		return await fetch(`${API.url}/users/${userId}`).then((res) =>
			res.json()
		);
	} catch (err) {
			console.error(err);
